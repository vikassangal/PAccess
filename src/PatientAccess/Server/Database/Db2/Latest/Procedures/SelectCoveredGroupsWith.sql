/*************I********************************************************/        
/*            I                                                       */        
/* iSeries400 I  SELECTCOVEREDGROUPSWITH - SQL PROC FOR PX	      */        
/*            I                                                       */        
/*    ************************************************************    */        
...
/* 02/18/2008 I	 Kevin Sedota      I  Start using search names        */ 
/*************I********************I***********************************/        
                                                                                
SET PATH *LIBL ;                                                                
                                                                                
CREATE PROCEDURE SELECTCOVEREDGROUPSWITH( 
	IN @P_COVEREDGROUPNAME VARCHAR(40),
		...
		FROM NKGC08P AS COVEREDGROUPS
			...
			AND TRIM(COVEREDGROUPS.GCGCOV) = TRIM(EMPLOYERS.EMNAME)
			AND COVEREDGROUPS.GCGCCD = EMPLOYERS.EMCODE * 100000
			
			--WHERE UPPER(TRIM(COVEREDGROUPS.GCGCOV)) 
			WHERE UPPER(TRIM(COVEREDGROUPS.GCGCSN)) 
			LIKE UPPER(TRIM(@P_COVEREDGROUPNAME)) || '%'
			AND FOLLOWUPUNITS.FFHSPN = @P_FACILITYID
			AND (	
