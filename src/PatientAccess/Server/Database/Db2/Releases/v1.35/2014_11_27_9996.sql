...
CREATE PROCEDURE SELECTGENERALACCTDATA ( 
	...
	P1 : BEGIN DECLARE CURSOR1 CURSOR FOR SELECT
	 ...
	 --D . MDRORG AS RESISTANTORGANISM , 
     D . MDMDFL AS MARKETINGEMAILOPTIN , 
	 ...
	 FROM HPDATA1 . HPADLPP A  ---- Patient P --hysical File (HPDATA1) 
...
LEFT JOIN NQHRLFP H  -- HRAM locks (XADATA) 
                   ON ( LPAD ( TRIM ( CHAR ( P_ACCOUNTNUMBER ) ) , 9 , '0' ) = SUBSTR ( H . LFKEY , 1 , 9 ) 
				   AND H . LFHSP# = P_HSP ) 
...
GRANT ALTER , EXECUTE   
ON SPECIFIC PROCEDURE SLSRGNACDT 
TO RMTGRPPRF ; 
