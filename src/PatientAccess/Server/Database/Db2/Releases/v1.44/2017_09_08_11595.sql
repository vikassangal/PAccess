...
--  Generated on:              	09/08/17 00:41:02 
--  Relational Database:       	DVLA 
--  Standards Option:          	DB2 for i 
SET PATH *LIBL ; 
  
CREATE PROCEDURE PADEV.SELECTGENERALACCTDATA ( 
	IN P_HSP INTEGER , 
	IN P_MRN INTEGER , 
	IN P_ACCOUNTNUMBER INTEGER ) 
	DYNAMIC RESULT SETS 1 
	LANGUAGE SQL 
	SPECIFIC PADEV.SLGNACDT 
	NOT DETERMINISTIC 
	MODIFIES SQL DATA 
...
JOIN NMDATA1.NOHLHSPP FAC  -- Hospital Definition Master (NMDATA1) 
ON ( FAC . HHHSPN = P_HSP ) 
JOIN HPDATA1.HPADMDP D  -- Patient Demographics (HPDATA1) 
ON ( D . MDHSP# = P_HSP 
AND D . MDMRC# = P_MRN ) 
LEFT OUTER JOIN HPDATA1.HPADQTOR Q  -- Resistant Organism (HPDATA1) 
ON ( Q . QTKEY = D . MDRORG ) 
LEFT OUTER JOIN HPDATA1.HPADLAP9 C  -- Patient comments/states aux file (HPDATA1) 
ON ( C . LAHSP# = P_HSP 
AND C . LAACCT = P_ACCOUNTNUMBER ) 
LEFT OUTER JOIN PADATA.PM0001P FUS  -- FUS Patient Master File (PADATA) 
ON ( FUS . PMHSPC = FAC . HHHSPC 
AND FUS . PMPT#9 = P_ACCOUNTNUMBER ) 
LEFT OUTER JOIN HPDATA1.HXMRRV2P SUP  -- Supplemental Abstracts file (HPDATA1) 
ON ( SUP . RVHSP# = P_HSP 
AND SUP . RVPT#9 = P_ACCOUNTNUMBER ) 
LEFT OUTER JOIN HPDATA1.HXMRRWP MPI  -- Master Patient Index (HPDATA1) 
ON ( MPI . RWHSP# = P_HSP 
AND MPI . RWACCT = P_ACCOUNTNUMBER ) 
LEFT OUTER JOIN HPDATA1.HXMRRVP RVP  -- Medical Records Abstracts Physical File (HPDATA1) 
ON ( RVP . RVHSP# = P_HSP 
AND RVP . RVACCT = P_ACCOUNTNUMBER ) 
LEFT JOIN XADATA.NQHRLFP H  -- HRAM locks (XADATA) 
ON ( LPAD ( TRIM ( CHAR ( P_ACCOUNTNUMBER ) ) , 9 , '0' ) 
= SUBSTR ( H . LFKEY , 1 , 9 ) 
AND H . LFHSP# = P_HSP ) 
LEFT OUTER JOIN NMDATA1.NDED01P EMER  -- Emergency Dept. log file (NMDATA1) 
ON ( EMER . EDHSP# = P_HSP 
...
GRANT ALTER , EXECUTE   
ON SPECIFIC PROCEDURE PADEV.SLGNACDT 
TO TURNOVER ;
