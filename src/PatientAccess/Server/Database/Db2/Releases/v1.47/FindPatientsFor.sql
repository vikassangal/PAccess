...
--  Generated on:              	08/14/18 10:11:48 
--  Relational Database:       	DVLA 
--  Standards Option:          	DB2 for i 
SET PATH *LIBL ; 
  
CREATE PROCEDURE PACCESS.FINDPATIENTSFOR ( 
...
DECLARE SEARCHCURSOR CURSOR WITH 
RETURN FOR SEARCHSTATEMENT ; 
SET @V_QUERYTEMPLATE = 
'SELECT DISTINCT                                         
...
1                                  AS TYPEOFNAMEID,                            
 ...
 TRIM ( DEMOGRAPHICS.MDPMI )        AS DISPLAYMIDDLEINITIAL,                    
  ...
  DEMOGRAPHICS.MDMADRE1               AS PHYSICALSTREET1EXTENDED,
 DEMOGRAPHICS.MDMADRE2               AS PHYSICALSTREET2EXTENDED,                      
  ...
  DEMOGRAPHICS.MDPADRE1            AS MAILINGSTREET1EXTENDED,   
DEMOGRAPHICS.MDPADRE2            AS MAILINGSTREET2EXTENDED,                          
...
|| @P_YEAR || ''') - 2 YEARS)           
AND                                                                          
LAST_DAY((DATE(''' || @P_MONTH || '/01/' 
|| @P_YEAR || ''') + 2 YEARS)) )          
 AND                                                                       
 ' || '( MONTH( 
 TEXTTODATE( DEMOGRAPHICS.MDDOB8 ) ) = ' 
|| @P_MONTH || ')' ; 
ELSEIF @P_YEAR IS NOT NULL THEN SET 
@V_DEMOFILTER = @V_DEMOFILTER || '           
AND ( TEXTTODATE( 
DEMOGRAPHICS.MDDOB8 )                                         
...
GRANT ALTER , EXECUTE   
ON SPECIFIC PROCEDURE PACCESS.FINDPTSFOR 
